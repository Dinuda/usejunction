...
test("usage daily refresh seals the UTC day, enqueues jobs, and drains", async () => {
  ...
  assert.equal(body.cachesInvalidated, undefined);
...
test("usage daily refresh rejects an invalid secret", async () => {
  ...
  assert.equal(mocks.drain.mock.calls.length, 0);
});
