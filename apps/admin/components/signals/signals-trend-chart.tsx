...
type Point = { weekStart: string; sessions: number; people: number; durationSeconds: number };
...
export function SignalsTrendChart({ data }: { data: Point[] }) {
  if (!data.length) {
    return <p className="py-8 text-sm text-muted-foreground">No weekly trend yet.</p>;
  }
    ...
    <ChartContainer config={chartConfig} className="h-[220px] w-full">
      {sparse ? (
        <BarChart data={data} margin={{ left: 0, right: 8, top: 12, bottom: 0 }} accessibilityLayer>
          ...
          <ChartTooltip content={<ChartTooltipContent labelFormatter={(value) => `Week of ${value}`} />} />
