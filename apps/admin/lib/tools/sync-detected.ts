...
/** Backfill org plans/assignments from already-reported installations and accounts. */
export async function syncDetectedPlansForOrg(orgId: string) {
  const [installations, accounts] = await Promise.all([
    prisma.toolInstallation.findMany({
      where: { orgId, detected: true },
      select: { userId: true, toolName: true },
    }),
    prisma.toolAccount.findMany({
      where: { orgId },
      select: { userId: true, toolName: true, plan: true, email: true },
    }),
  ]);

  const byDeveloper = new Map<string, { toolNames: Set<string>; accounts: DetectedAccount[] }>();

  for (const installation of installations) {
    const entry = byDeveloper.get(installation.userId) ?? { toolNames: new Set(), accounts: [] };
    entry.toolNames.add(installation.toolName);
    byDeveloper.set(installation.userId, entry);
  }

  for (const account of accounts) {
    const entry = byDeveloper.get(account.userId) ?? { toolNames: new Set(), accounts: [] };
    entry.toolNames.add(account.toolName);
    entry.accounts.push({
      toolName: account.toolName,
      plan: account.plan,
      email: account.email,
    });
    byDeveloper.set(account.userId, entry);
  }

  let created = 0;
  let assigned = 0;
  for (const [developerId, data] of byDeveloper) {
    const result = await syncDetectedPlansForDevice({
      orgId,
      developerId,
      toolNames: Array.from(data.toolNames),
