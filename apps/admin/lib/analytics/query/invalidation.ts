 ...
 * overlaps those days. Rematerialize is opt-in (Sync now / cron / first-sync
 * policy) — default is dirty-only so background ingest does not block on CTEs.
 */
...
export async function invalidateAnalyticsCache(
  ...
  try {
    ...
    // Invalidate caches whose window overlaps any dirty day (contiguous bounding box).
    ...
    return { marked: [] as string[], rematerialized: false };
  }
}
