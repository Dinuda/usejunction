...
const PAID_STATUSES = new Set(["active", "on_trial", "cancelled", "paused"]);

export function resolveEffectivePlan(org: OrgBillingFields): SaasPlan {
  const plan = org.plan as SaasPlan;

  if (plan === "enterprise" && org.subscriptionStatus && PAID_STATUSES.has(org.subscriptionStatus)) {
    return "enterprise";
  }

  if (plan === "team" && org.subscriptionStatus && PAID_STATUSES.has(org.subscriptionStatus)) {
    return "team";
  }
