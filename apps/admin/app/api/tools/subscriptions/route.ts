...
export async function POST(req: NextRequest) {
  ...
  return NextResponse.json(serializeBigInts({ subscription: { ...plan, assignedSeats: 0, availableSeats: plan.seatCapacity, estimatedCycleMicros: plan.cycleSeatMicros * BigInt(plan.seatCapacity) } }), { status: 201 });
}
