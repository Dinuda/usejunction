...
export function JoinInviteButton({ token }: { token: string }) {
  ...
  if (error) return <div className="space-y-4"><Alert variant="destructive"><AlertDescription>{error}</AlertDescription></Alert><Button variant="outline" onClick={() => { resetPostHogIdentity(); return signOut({ callbackUrl: `/join/${token}` }); }}>Use a different account</Button></div>;
